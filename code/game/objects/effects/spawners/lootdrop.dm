	...
	if(loot && loot.len)
		...
		while((lootcount-loot_spawned) && loot.len)
			...
			if(lootspawn)
				...
				if (!fan_out_items)
					spawned_loot.forceMove(T, step_x, step_y)
				else if (loot_spawned)
					var/step_vars = ((!(loot_spawned%2)*loot_spawned/2)*-1)+((loot_spawned%2)*(loot_spawned+1)/2*1)
					spawned_loot.forceMove(T, step_vars, step_vars)
			loot_spawned++
